import Card from "react-bootstrap/Card";
import Button from "react-bootstrap/Button";

const CoursesSection = () => {
  const dummyCourses = [
    {
      courseID: 1,
      courseTitle: "Podstawy programowania w języku Python",
      courseDescripton:
        "Kurs dla początkujących, wprowadzający do podstawowych pojęć i technik programowania w języku Python.",
      courseImage:
        "https://www.python.org/static/community_logos/python-logo-master-v3-TM-flattened.png",
    },
    {
      courseID: 2,
      courseTitle: "Tworzenie stron internetowych z HTML i CSS",
      courseDescripton:
        "Nauka tworzenia responsywnych stron internetowych przy użyciu języków HTML i CSS.",
      courseImage:
        "https://i.pinimg.com/originals/d5/bc/80/d5bc803f0ab768736cb0df5c06109c9a.png",
    },
    {
      courseID: 3,
      courseTitle: "Programowanie aplikacji mobilnych w języku Java",
      courseDescripton:
        "Kurs skupiający się na tworzeniu aplikacji mobilnych dla systemu Android przy użyciu języka Java.",
      courseImage: "https://logowik.com/content/uploads/images/731_java.jpg",
    },
    {
      courseID: 4,
      courseTitle: "Zaawansowane programowanie w języku C++",
      courseDescripton:
        "Kurs dla programistów, którzy chcą pogłębić swoją wiedzę na temat języka C++ i nauczyć się zaawansowanych technik programowania.",
      courseImage:
        "https://upload.wikimedia.org/wikipedia/commons/thumb/1/18/ISO_C%2B%2B_Logo.svg/1822px-ISO_C%2B%2B_Logo.svg.png",
    },
    {
      courseID: 5,
      courseTitle: "Tworzenie aplikacji webowych przy użyciu frameworka React",
      courseDescripton:
        "Nauka tworzenia nowoczesnych aplikacji webowych przy użyciu biblioteki React oraz narzędzi z ekosystemu React.",
      courseImage:
        "https://www.datocms-assets.com/45470/1631110818-logo-react-js.png",
    },
    {
      courseID: 6,
      courseTitle: "Analiza danych i uczenie maszynowe w języku R",
      courseDescripton:
        "Kurs skupiający się na wykorzystaniu języka R do analizy danych i uczenia maszynowego, w tym technik takich jak regresja i klasyfikacja.",
      courseImage:
        "https://upload.wikimedia.org/wikipedia/commons/thumb/1/1b/R_logo.svg/2560px-R_logo.svg.png",
    },
    {
      courseID: 7,
      courseTitle: "Tworzenie interaktywnych stron internetowych z JavaScript",
      courseDescripton:
        "Nauka tworzenia interaktywnych i dynamicznych stron internetowych przy użyciu języka JavaScript oraz popularnych bibliotek takich jak jQuery.",
      courseImage:
        "https://miro.medium.com/v2/resize:fit:1024/1*tsqlgfBBb32ZLx1Y6bwzKA.jpeg",
    },
    {
      courseID: 8,
      courseTitle: "Tworzenie aplikacji mobilnych w środowisku Flutter",
      courseDescripton:
        "Kurs skupiający się na tworzeniu aplikacji mobilnych dla systemów Android i iOS przy użyciu frameworka Flutter.",
      courseImage:
        "data:image/png;base64,",
    },
    {
      courseID: 9,
      courseTitle: "Projektowanie baz danych i SQL",
      courseDescripton:
        "Nauka projektowania efektywnych baz danych oraz wykorzystywania języka SQL do manipulacji danymi i wykonywania zapytań.",
      courseImage:
        "https://upload.wikimedia.org/wikipedia/commons/8/87/Sql_data_base_with_logo.png",
    },
    {
      courseID: 10,
      courseTitle:
        "Tworzenie aplikacji webowych przy użyciu frameworka Node.js",
      courseDescripton:
        "Kurs skupiający się na tworzeniu aplikacji webowych i API przy użyciu środowiska Node.js oraz frameworka Express.",
      courseImage: "https://www.vectorlogo.zone/logos/nodejs/nodejs-ar21.png",
    },
  ];

  return (
    <div>
      CoursesSection
      {dummyCourses.map(({ courseTitle, courseDescripton, courseImage }) => (
        <Card style={{ padding: "5rem", margin: "5rem" }}>
          <Card.Img
            variant="top"
            style={{ maxHeight: "10rem", maxWidth: "10rem" }}
            src={courseImage}
          />
          <Card.Body>
            <Card.Title>{courseTitle}</Card.Title>
            <Card.Text>{courseDescripton}</Card.Text>
            <Button variant="primary">Zapisz się na kurs!</Button>
          </Card.Body>
        </Card>
      ))}
    </div>
  );
};

export default CoursesSection;
